...
| 模式 | 能画什么 |
| --- | --- |
| 函数图像 | 直角坐标、极坐标、参数方程、隐函数 `f(x,y)=c`、不等式区域、导数/积分曲线、数列散点；多图层、线宽/虚线/填充/采样数逐个可调 |
...
顶栏「载入示例」内置 28 个预设，覆盖上述每一类；控制台模式可直接写 `f(x) = …`、`k = …` 定义用户函数与常量，全部模式共享。
...
src/presets.ts 28 个示例预设
scripts/       verify.ts —— 154 项内核回归断言
```
